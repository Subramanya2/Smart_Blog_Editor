...
import { Layout, Plus, FileText, LogOut, Trash2, Search, Sparkles, Wifi } from 'lucide-react';

export default function Sidebar({ user, posts, activePostId, onSelectPost, onCreatePost, onDeletePost, onLogout }) {
    const [searchQuery, setSearchQuery] = useState('');

    const filteredPosts = posts.filter(post =>
        (post.title || 'Untitled Draft').toLowerCase().includes(searchQuery.toLowerCase())
    );
        ...
        <aside className="w-72 bg-slate-900 border-r border-slate-800/80 flex flex-col h-full select-none text-slate-300">
            ...
            <div className="px-4 py-2">
                <div className="relative">
                    ...
                    <input
                        ...
                        className="w-full bg-slate-950/60 border border-slate-800 rounded-lg pl-8 pr-3 py-1.5 text-xs text-slate-200 placeholder-slate-500 focus:outline-none focus:border-purple-500/60 focus:ring-1 focus:ring-purple-500/40 transition"
                    />
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto px-3 py-2 space-y-1">
                <div className="px-2 pb-1 text-[11px] font-semibold text-slate-500 uppercase tracking-wider flex items-center justify-between">
                    <span>Documents</span>
                    <span className="text-[10px] font-mono bg-slate-800/80 px-1.5 py-0.2 rounded text-slate-400">{filteredPosts.length}</span>
                </div>

                {filteredPosts.length === 0 ? (
                    <div className="text-center py-8 text-xs text-slate-500">
                        {searchQuery ? 'No drafts found' : 'No drafts yet. Click "New Draft" to begin!'}
                    </div>
                ) : (
